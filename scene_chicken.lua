---------------------------------------------------------------------------------
-- Chicken Scene
-- This scene will display the chicken graphic and instruct user to shake the phone after asking a question
---------------------------------------------------------------------------------
...
function scene:createScene( event )
  ...
  sceneTitle.y = display.contentCenterY - 60

  group:insert( sceneTitle )

  local sceneInstr = display.newText( "Shake to get your Answer!", 0, 0, globals.font.bold, 14 )
  sceneInstr.x = display.contentCenterX
  sceneInstr.y = display.contentCenterY + 20
  
  group:insert( sceneInstr )
...
function scene:enterScene( event )
  ...
  local function onShake (event)
    if event.isShake then
        audio.stop( chickenSoundsChannel )
        audio.play(chicken_squawk)
        Runtime:removeEventListener( "accelerometer", onShake )
        -- Device was shaken, so call function to switch to answer scene
        switchToAnswerScene()
    end
  end
